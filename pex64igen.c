...
_bfd_pex64i_swap_scnhdr_out (bfd * abfd, void * in, void * out)
{
  ...
  if (coff_data (abfd)->link_info
      ...
      && memcmp (scnhdr_int->s_name, ".text", sizeof ".text") == 0)
    {
	 ...
	 bunch of other fields long long before this!  */
...
CODEVIEW_INFO *
_bfd_pex64i_slurp_codeview_record (bfd * abfd, file_ptr where, unsigned long long length, CODEVIEW_INFO *cvinfo,
				char **pdb)
{
  char buffer[256+1];
  bfd_size_type nread;
...
pe_print_edata (bfd * abfd, void * vfile)
{
  ...
  /* The next table to find is the Export Address Table. It's basically
     ...
     forward the call to another dll. Something like:
      typedef union
      {
	long long export_rva;
	long long forwarder_rva;
			   ...
			   rsrc_entry * b ATTRIBUTE_UNUSED)
{
  ...
  BFD_ASSERT (nstring - new_data == (int) (a->value.leaf->size + copy_needed));
